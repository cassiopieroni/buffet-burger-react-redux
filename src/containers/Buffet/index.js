...
export default () => {
    ...
    const handleClickAddDishToBag = () => {
        ...
        if (isValid) {
            dispatch( addProductToBag({ dishItems: dish.summaryItems, dishPrice: dish.subtotal }));
            dispatch( addMessage({ type: 'success', content: 'Prato adicionado na sacola de compras!' }));
            dispatch( clearDish());
        }

        else 
            errorMessages.forEach( msg => dispatch( addMessage({ type: 'error', content: msg }) ));
    }
