...
const DishItem = ({ item }) => (
    ...
    <li>
        ...
        <span><NumToReal num={ item.price } /></span>
    </li> 
);
    ...
    <StyledSection>
        ...
        <div>
            ...
            {( dishSummary && dishSummary.length ) ? (
                <>
                    <ul>
                        { dishSummary.map((item, i) => <DishItem key={ i } item={ item } /> )}
                    </ul>

                    <StyledP1>Total: <span><NumToReal num={subtotal} /></span></StyledP1>
                </>
            ) : (
            ...
            <NavLink to='/shopping-bag'>
                sacola de compras ( { productsLength } )
            </NavLink>
